...
bool authenticate(resp_https_t response, req_https_t request) {
  ...
  auto ip_type = net::from_address(address);
   ...
   //Skip passwords and all that business
  return true;
...
void getAutoPinPage(resp_https_t response, req_https_t request) {
 // if(!authenticate(response, request)) return;

  print_req(request);

  std::string content = read_file(WEB_DIR "autopin.html");
...
void start() {
 ...
 // server.resource["^/pin$"]["GET"]                                 = getPinPage;
  server.resource["^/autopin$"]["GET"]                                 = getAutoPinPage;
